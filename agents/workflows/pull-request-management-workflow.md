...
3. Create PRs as drafts. Before pushing changes to an existing open PR, convert it back to draft unless the user asks to keep it ready for review.
   When the follow-up work resolves review comments, use `agents/workflows/pull-request-comment-resolution-workflow.md` and restore ready-for-review status after resolved thread state is verified unless the PR was intentionally left draft.
...
- Write PR titles in title case. Read the first available title casing reference: local `agents/references/title-casing-reference.md`, then packaged `skill://pull-request-management/title-casing.md`.
- For UI changes, include screenshots or explain why screenshots do not apply.
- When screenshots are required, use `agents/workflows/upload-pr-screenshots-workflow.md` for upload and PR body formatting.
