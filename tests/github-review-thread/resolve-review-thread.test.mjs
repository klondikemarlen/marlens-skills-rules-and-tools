...
await test('creates the expected reaction before resolving and verifies both states', async () => {
  ...
  assert.deepEqual(
    {
      result,
      restCalls: restCalls.map(({ method, endpoint, payload }) => ({
        method,
        endpoint,
        payload,
      })),
      graphCalls: graphCalls.map(({ query, variables }) => ({
        query,
        variables,
      })),
    },
    {
      ...
      restCalls: [
        ...
        {
          method: 'GET',
          endpoint: `${reviewCommentReactionEndpoint('owner/repo', 123)}?per_page=100&page=1`,
          payload: undefined,
        },
        {
          method: 'POST',
          endpoint: reviewCommentReactionEndpoint('owner/repo', 123),
          payload: { content: '-1' },
        },
        {
          method: 'GET',
      ...
      graphCalls: [
        ...
        {
          query: graphqlResolveMutation(),
          variables: { threadId: 'thread-1' },
        },
        {
          query: graphqlThreadVerifyQuery(),
          variables: { threadId: 'thread-1' },
        },
      ],
    },
...
await test('repairs a missing reaction on an already-resolved thread without resolving twice', async () => {
  ...
  assert.deepEqual(
    {
      result,
      graphQueries: graphCalls.map(({ query }) => query),
      reactionWrite: restCalls[2],
    },
    {
      result: {
...
await test('rejects completion when the final reaction check is missing', async () => {
  const restResponses = [{ id: 42 }, [], { id: 101, content: '+1' }, [{ content: '+1', user: { id: 42 } }], []];
  const graphResponses = [
    {
      data: {
        repository: {
  ...
  await assert.rejects(github.resolveReviewThread('owner/repo', 456, 123, '+1'), /Completion verification failed/u);
});
