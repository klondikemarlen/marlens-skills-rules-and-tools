...
await test('rejects an external repository before any dry-run plan', () => {
  ...
  assert.deepEqual({ status: result.status, error: result.stderr.trim() }, {
    status: 1,
    error: 'github-review-thread: Refusing external target: evil/repo != checkout repository klondikemarlen/marlens-skills-rules-and-tools',
  });
});
...
await test('previews accepted feedback with the final verification operations', () => {
  ...
  assert.deepEqual({ status: result.status, reaction: output.reaction, operationCount: output.plan.length, finalQuery: output.plan.at(-1).query }, {
    status: 0,
    reaction: '+1',
    operationCount: 8,
