...
| [`omp-github-write-guard`](https://github.com/klondikemarlen/omp-github-write-guard) | One standard OMP Ask for recognized external `git push` and GitHub issue creation; one exact retry. It does not guard PR/API writes or writes outside OMP. | `omp plugin install github:klondikemarlen/omp-github-write-guard` | You do not need opt-in push/issue-creation enforcement. |
