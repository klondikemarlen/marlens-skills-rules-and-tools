...
When the user says **OMP**, use OMP plugin docs, OMP plugin install commands such as `omp plugin install <source>`, OMP plugin manifests such as `omp.extensions` or `plugin.json`, and OMP paths under `~/.omp/...`.
