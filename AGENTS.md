...
- When installed, the opt-in `klondikemarlen/omp-repository-boundary-guard` project provides repository-boundary guarding for local writes and moves, `git push`, supported `gh issue`/`gh pr`/`gh api` mutations, and supported `xd://github` writes. Install the released boundary guard with `omp plugin install github:klondikemarlen/omp-repository-boundary-guard#v1.0.0`.
  It requires one standard OMP Ask before a mutation crosses the active repository boundary; approval permits one exact retry. Its absence does not change the same-origin allowance.
