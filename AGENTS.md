...
- When installed, the opt-in `klondikemarlen/omp-soft-boundary-guard` project logs advisory warnings for recognized local writes and moves, `git push`, supported `gh issue`/`gh pr`/`gh api` mutations, and supported `xd://github` writes that resolve outside the active boundary. Routine installs use `omp plugin install github:klondikemarlen/omp-soft-boundary-guard` and follow the default branch.
  Use `#<full-commit-hash> --force` only to reproduce an exact artifact or diagnose stale plugin-cache state. The installed extension never blocks or asks; programmatic callers that opt into `createRepositoryBoundaryGuard({ enforce: true })` receive one standard OMP Ask before a resolved cross-boundary mutation, with approval permitting one exact retry. Its absence does not change the same-origin allowance.
