...
export async function uploadPullRequestBodyScreenshots({
  ...
  validateArguments({ page, prUrl, editorSelector, bodyOptionsSelector, fileInputSelector, screenshots });

  await page.goto(prUrl, { waitUntil: 'domcontentloaded' });
  await requireAuthenticatedGitHub(page);
  const editor = await openPrBodyEditor(page, editorSelector, bodyOptionsSelector, timeoutMs);
...
function validateArguments({ page, prUrl, editorSelector, bodyOptionsSelector, fileInputSelector, screenshots }) {
...
async function openPrBodyEditor(page, editorSelector, bodyOptionsSelector, timeoutMs) {
  const existingEditor = await page.$(editorSelector);
  if (existingEditor) return existingEditor;
  if (!bodyOptionsSelector) {
    throw new Error('bodyOptionsSelector is required when the PR body editor is closed.');
  }

  const options = await page.$(bodyOptionsSelector);
  if (!options) throw new Error(`PR body options control not found: ${bodyOptionsSelector}.`);

  const menuId = await options.evaluate((element) => element.getAttribute('aria-controls'));
  if (!menuId) throw new Error('PR body options control must identify its menu with aria-controls.');
  await options.click();
  const menuSelector = `[id="${escapeAttributeValue(menuId)}"]`;
