...
const ATTACHMENT_PATTERN =
  /(?:<img\b[^>]*?src=["'](?<htmlUrl>https:\/\/github\.com\/user-attachments\/assets\/[^"']+)["'][^>]*>|!\[[^\]]*\]\((?<markdownUrl>https:\/\/github\.com\/user-attachments\/assets\/[^)]+)\))/i
...
export async function addImageToGitHubMarkdownEditor({
  ...
  if (!input) {
    throw new Error(
      `No file input matching ${fileInputSelector} was scoped to editor ${editorSelector}.`
    )
  }
  ...
  const markdown = await waitForAttachmentMarkdown({
    page,
    editor,
    before,
    baselineErrorText,
  ...
  if (!attachmentUrl)
    throw new Error(
      `GitHub upload changed the editor but no user-attachments/assets URL was found.`
    )
...
async function prepareEditor(editor, insertAt) {
  const result = await editor.evaluate((element, placeholder) => {
    ...
    if (element.disabled || element.readOnly) {
      return {
        error: "Target GitHub Markdown editor is disabled or read-only.",
      }
    }
    ...
    if (placeholder) {
      ...
      if (start === -1)
        return {
          error: `Insert placeholder not found in target editor: ${placeholder}`,
        }
...
async function findScopedFileInput(page, editor, fileInputSelector) {
  ...
  for (const input of inputs) {
    const isScoped = await page.evaluate(
      (editorElement, inputElement) => {
        ...
        const container =
          editorElement.closest?.(
            '.js-previewable-comment-form, .js-comment-container, .js-discussion, [data-target*="markdown"]'
...
async function waitForAttachmentMarkdown({ page, editor, before, baselineErrorText, timeoutMs }) {
  ...
  while (Date.now() < deadline) {
    latest = await editor.evaluate((element) =>
    ...
    const errorText = newUploadErrorText(
      await gitHubUploadErrorText(page, editor),
      baselineErrorText
    )
  ...
  if (errorText) throw new Error(`GitHub image upload failed: ${errorText}`)
  if (latest === before)
    throw new Error("GitHub image upload timed out and the target editor value did not change.")

  throw new Error(
    "GitHub image upload changed the editor but did not insert user-attachments/assets Markdown."
  )
}
...
function attachmentMarkdownMatches(value) {
  return [...String(value || "").matchAll(new RegExp(ATTACHMENT_PATTERN.source, "gi"))].map(
    (match) => match[0]
  )
}
