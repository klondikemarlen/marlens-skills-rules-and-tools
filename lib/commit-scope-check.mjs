...
function atomicGroupBoundary(paths, atomicGroups) {
  ...
  const exactGroup = normalizedGroups.find((group) => (
...
export function checkCommitScope(paths, { atomicGroups = [], composeDockerfilePairs = [] } = {}) {
  ...
  if (atomicGroup) {
    boundaries.push({ message: 'Atomic commit groups must include every member and no other path:', ...atomicGroup });
  }
...
export function formatCommitScopeFailures(boundaries) {
  const details = boundaries.flatMap(({ categories, message, paths }) => [
    ...
    ...(categories ?? Object.keys(paths)).flatMap((category) => paths[category].map((filePath) => `  - ${category}: ${filePath}`)),
  ]);
