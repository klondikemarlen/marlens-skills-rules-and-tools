...
function isDocumentation(filePath) {
  ...
  return (
    normalizedPath(filePath).startsWith("docs/") ||
...
function isMigration(filePath) {
  return normalizedPath(filePath)
    .split("/")
    .some((segment) => /^(migrations?|migrate)$/u.test(segment))
}
...
function isDeveloperTooling(filePath) {
  ...
  return (
    /^(bin|scripts|tools|dev|\.github)\//u.test(path) ||
...
function isConfiguration(filePath) {
  ...
  return (
    /^\.env(?:\..+)?$/u.test(name) ||
    /(^|\.)config\.[^.]+$/u.test(name) ||
    /^(package|tsconfig|jsconfig|vite|vitest|eslint|prettier|babel|webpack|jest)\.json$/u.test(
      name
    ) ||
    [
      "bun.lock",
...
function atomicGroupBoundary(paths, atomicGroups) {
  const stagedPaths = new Set(paths.map(normalizedPath))
  const normalizedGroups = atomicGroups.map((group) => group.map(normalizedPath))
  const exactGroup = normalizedGroups.find(
    (group) =>
      group.length === stagedPaths.size && group.every((filePath) => stagedPaths.has(filePath))
  )

  if (exactGroup) return { exact: true }

  const group = normalizedGroups.find((candidate) =>
    candidate.some((filePath) => stagedPaths.has(filePath))
  )
  if (!group) return null

  return {
    paths: {
      "declared group": group,
      "staged paths": [...stagedPaths],
    },
  }
}

function composeDockerfilePairBoundary(paths, composeDockerfilePairs) {
  const stagedPaths = new Set(paths.map(normalizedPath))
  const related = new Set()

  for (const { composePath, dockerfilePath } of composeDockerfilePairs) {
    const normalizedComposePath = normalizedPath(composePath)
    const normalizedDockerfilePath = normalizedPath(dockerfilePath)
    if (!stagedPaths.has(normalizedComposePath) || !stagedPaths.has(normalizedDockerfilePath))
      continue

    related.add(normalizedComposePath)
    related.add(normalizedDockerfilePath)
  }

  if (related.size === 0) return null
  if (related.size === 2 && related.size === stagedPaths.size) return { exact: true }

  return {
    message: "Compose Dockerfile commits must include exactly one directly referenced pair:",
    paths: {
      "Compose Dockerfile pairs": [...related],
      "other staged paths": [...stagedPaths].filter((filePath) => !related.has(filePath)),
    },
  }
}

export function checkCommitScope(paths, { atomicGroups = [], composeDockerfilePairs = [] } = {}) {
  const categories = pathsByCategory(paths)
  const atomicGroup = atomicGroupBoundary(paths, atomicGroups)
  const composeDockerfilePair = composeDockerfilePairBoundary(paths, composeDockerfilePairs)

  if (atomicGroup?.exact || composeDockerfilePair?.exact) return { categories, boundaries: [] }
  ...
  if (atomicGroup) {
    boundaries.push({
      message: "Atomic commit groups must include every member and no other path:",
      ...atomicGroup,
    })
  }
  if (composeDockerfilePair) {
    boundaries.push(composeDockerfilePair)
  }

  const configurationPaths = categories.get("configuration")
  const nonConfigurationPaths = CATEGORY_ORDER.filter(
...
export function formatCommitScopeFailures(boundaries) {
  const details = boundaries.flatMap(({ categories, message, paths }) => [
    `- ${message ?? `${categories.map((category) => CATEGORY_LABELS[category] ?? category).join(" and ")} must be separate:`}`,
    ...(categories ?? Object.keys(paths)).flatMap((category) =>
      paths[category].map((filePath) => `  - ${category}: ${filePath}`)
    ),
  ])
