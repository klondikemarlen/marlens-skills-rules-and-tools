...
function composeDockerfilePairBoundary(paths, composeDockerfilePairs) {
  ...
  if (related.size === stagedPaths.size) return { exact: true };

  return {
    message: 'Compose Dockerfile pairs must be staged without unrelated paths:',
    paths: {
      'Compose Dockerfile pairs': [...related],
      'unrelated staged paths': [...stagedPaths].filter((filePath) => !related.has(filePath)),
    },
  };
}
