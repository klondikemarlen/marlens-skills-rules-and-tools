...
function atomicGroupBoundary(paths, atomicGroups) {
  ...
  const exactGroup = normalizedGroups.find(
    (group) => group.length === stagedPaths.size && group.every((filePath) => stagedPaths.has(filePath)),
  );

  if (exactGroup) return { exact: true };

  const group = normalizedGroups.find((candidate) => candidate.some((filePath) => stagedPaths.has(filePath)));
...
function composeDockerfilePairBoundary(paths, composeDockerfilePairs) {
  ...
  for (const { composePath, dockerfilePath } of composeDockerfilePairs) {
    ...
    if (!stagedPaths.has(normalizedComposePath) || !stagedPaths.has(normalizedDockerfilePath)) continue;
...
export function checkCommitScope(paths, { atomicGroups = [], composeDockerfilePairs = [] } = {}) {
  ...
  const nonConfigurationPaths = CATEGORY_ORDER.filter((category) => category !== 'configuration').flatMap((category) =>
