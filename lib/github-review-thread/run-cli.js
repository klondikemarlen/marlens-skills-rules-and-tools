...
export async function runCli(argv, cwd, environment) {
  try {
    ...
    const pullRequest = action === 'reply' || action === 'resolve' ? parseInteger(options.pr, '--pr') : undefined;
    if (action === 'resolve') options.reaction = parseReaction(options);

    if (options.dryRun) {
      const [owner, name] = repository.split('/');
      const plan = await buildPlan(action, repository, commentId, { ...options, pr: pullRequest }, owner, name);
      console.log(
        JSON.stringify({
          action,
          repository,
