...
export async function runCli(argv, cwd, environment) {
  try {
    ...
    if (!currentRepository)
      throw new Error("Could not resolve current checkout GitHub repository origin.")
    if (repository !== currentRepository) {
      throw new Error(
        `Refusing external target: ${repository} != checkout repository ${currentRepository}`
      )
    }

    const commentId = parseInteger(options.commentId, "--comment-id")
    const pullRequest =
      action === "reply" || action === "resolve" ? parseInteger(options.pr, "--pr") : undefined
    ...
    if (!token)
      throw new Error("Authenticate with gh (`gh auth login`) or set GITHUB_TOKEN/GH_TOKEN.")
    ...
    const result = await github.resolveReviewThread(
      repository,
      pullRequest,
      commentId,
      options.reaction
