...
export class GitHubReviewIntegration {
  ...
  async resolveReviewThread(repository, pullRequest, commentId, reaction) {
    ...
    if (!mapped) {
      throw new Error(
        `No review-thread mapping found for comment ${commentId} in ${repository}#${pullRequest}.`
      )
    }
    ...
    const verification = await this.verifyThreadCompletion(
      repository,
      commentId,
      reaction,
      viewerId,
  ...
  async applyReaction(repository, commentId, reaction) {
    ...
    const alreadyReacted = await this.currentViewerHasReaction(
      repository,
      commentId,
      reaction,
      viewerId
    ...
    if (!alreadyReacted) {
      ...
      if (!(await this.currentViewerHasReaction(repository, commentId, reaction, viewerId))) {
        throw new Error(
          `Reaction ${reaction} was not present after the request for comment ${commentId}.`
        )
      }
    }
  ...
  async currentViewerHasReaction(repository, reviewCommentId, expectedReaction, viewerId) {
    for (let page = 1; ; page += 1) {
      ...
      if (!Array.isArray(reactions)) {
        throw new Error(
          `Reaction lookup returned an invalid response for comment ${reviewCommentId}.`
        )
      }
