...
- Run the smallest existing read-only audit and behavior check relevant to the finding before proposing a change. For this package's own checkout, use `node bin/agent-guidance-audit.js --strict .` for guidance integrity and `node scripts/verify-skill-workflows.mjs` for skill wiring.
