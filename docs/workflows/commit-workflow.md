...
4. Run `check-commit-scope` after staging. If it reports a mixed file-type boundary, stop and split the named paths. For a genuinely inseparable feature or release group, declare the exact all-or-nothing paths in the repository-root `.commit-scope.json`; partial groups and unrelated extra paths still fail. Use `check-commit-scope --allow-mixed` only after the user explicitly confirms that the categories are genuinely inseparable, then record the exception in the commit body.
