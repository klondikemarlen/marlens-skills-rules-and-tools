...
Check candidate existence first (for example with glob). Read only the first existing local workflow file; if neither local workflow exists, read `skill://git-rebase/workflow.md` directly. Do not read missing local paths or batch local candidates.
