...
description: Use when adding backend features in Node.js + Express + TypeScript projects that follow route, controller, service, policy, serializer, model, and test rails.
---
...
3. Packaged fallback: [workflow.md](workflow.md) in this skill directory (`skill://express-light-rail/workflow.md` in OMP)
Use a single non-erroring project-root glob with the candidate basename (for example, `glob("**/express-light-rail-backend-workflow.md")`) to find existing local candidates; choose the listed `docs/workflows/` path before its `agents/workflows/` counterpart. If neither local workflow exists, read `skill://express-light-rail/workflow.md` directly. Do not probe missing literal paths or read them speculatively.
