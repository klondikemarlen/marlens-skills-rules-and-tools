...
4. Run `check-commit-scope` after staging. A standard source-plus-focused-test commit does not use `.commit-scope.json`; its absence is expected, so do not probe it or report it as a repository failure. Only after the checker reports a mixed file-type boundary and the user confirms that the group is genuinely inseparable may you read or create this optional exception configuration. Then declare the exact all-or-nothing paths in the repository-root `.commit-scope.json`; partial groups and unrelated extra paths still fail. Use `check-commit-scope --allow-mixed` only after that explicit confirmation, then record the exception in the commit body.
   In an OMP plugin session, this package makes `check-commit-scope` available on `PATH`; repo-local commit guidance may still require its own invocation.
