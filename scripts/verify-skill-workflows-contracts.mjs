...
export function skillContract(read, skillName) {
  ...
  const local = [...text.matchAll(/`((?:docs|agents)\/workflows\/[^`]+)`/g)].map(
    (match) => match[1]
  )
...
export function verifyWorkflowContracts({ root, read, fail }) {
  ...
  for (const entry of readdirSync(path.join(root, "skills"), {
    ...
    if (local.length !== packaged.length * 2)
      fail(`${entry.name}: local and packaged workflow paths must align`)
    ...
    for (let index = 0; index < local.length; index += 2) {
      ...
      if (!existsSync(path.join(root, preferred)))
        fail(`${entry.name}: missing preferred workflow ${preferred}`)
    ...
    for (const fallback of packaged) {
      ...
      if (!existsSync(fallbackPath(fallback.uri)))
        fail(`${entry.name}: missing skill://${fallback.uri}`)
    }
  }

  const sessionInsightWorkflows = [
    [
      "authoritative session insight workflow",
      read("docs/workflows/session-insight-mining-workflow.md"),
    ],
  ...
  if (
    expressWorkflow.includes(
      "Pick only the templates needed from `docs/templates/backend/express-light-rail/`"
    )
  ) {
  ...
  const selfImprovementWorkflows = [
    [
      "authoritative self-improvement workflow",
      read("docs/workflows/self-improvement-workflow.md"),
    ],
