...
try {
  ...
  const scopedInput = buildInput(scopedForm, () =>
    replaceSelection(editorWithExisting, newMarkdown)
  )
  ...
  assert.equal(
    existingResult.attachmentUrl,
    "https://github.com/user-attachments/assets/newer-upload"
  )
  ...
  assert.equal(
    baselineResult.attachmentUrl,
    "https://github.com/user-attachments/assets/baseline-upload"
  )
