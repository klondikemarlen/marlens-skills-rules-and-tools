...
if (existsSync(path.join(root, ".claude-plugin", "plugin.json"))) {
  ...
  if (claudePluginJson.version !== packageJson.version)
    fail("Claude plugin version must match package version")
}
if (existsSync(path.join(root, ".claude-plugin", "marketplace.json"))) {
  const claudeMarketplaceJson = JSON.parse(read(".claude-plugin/marketplace.json"))
  if (
    !claudeMarketplaceJson.plugins?.some(
      (plugin) => plugin.name === packageJson.name && plugin.source === "./"
    )
  ) {
...
if (existsSync(path.join(root, ".omp-plugin", "marketplace.json"))) {
  ...
  if (ompMarketplaceJson.name !== packageJson.name)
    fail("OMP marketplace name must match package name")
  if (ompMarketplaceJson.plugins?.length !== 1)
    fail("OMP marketplace must expose exactly one plugin entry")
  const [ompPlugin] = ompMarketplaceJson.plugins ?? []
  if (ompPlugin) {
    if (ompPlugin.name !== packageJson.name)
      fail("OMP marketplace plugin name must match package name")
    if (ompPlugin.source !== "./")
      fail("OMP marketplace plugin source must install this repo as the plugin root")
    if (
      normalizedRepositoryUrl(ompPlugin.repository) !==
      normalizedRepositoryUrl(packageJson.repository.url)
    ) {
...
const administrationTabTemplate = read(
  "docs/templates/frontend/administration-tab-page-template.md"
)
...
const notificationEventTemplate = read(
  "docs/templates/backend/notification-event-service-template.md"
)
...
const downstreamAuditReference = read(
  "docs/references/downstream-agent-guidance-audit-reference.md"
)
for (const requiredText of [
...
const backendCrudTemplate = read(
  "docs/templates/backend/express-sequelize-crud/resource-rail-template.md"
)
const frontendCrudTemplate = read(
  "docs/templates/frontend/vue-vuetify-crud/admin-resource-pages-template.md"
)
const searchableAutocompleteTemplate = read(
  "docs/templates/frontend/searchable-autocomplete-template.md"
)
...
if (
  apiTypescriptTemplate.includes("export const STATUSES") ||
  apiTypescriptTemplate.includes("  STATUSES,")
) {
...
if (
  !referencesIndex.includes("Code organization, module boundaries, and pattern-selection criteria")
) {
...
for (const requiredText of [
  ...
  "one project-root import convention",
  "`@/…`",
  "When raw Node runs ESM source directly",
  "`#/…` package-import namespace",
  '`"#/*": "./*"`',
  "Node v24.14.0 or later",
  "documented valid named `#` namespace",
  "When a directory such as `tests` is a sibling of the source root",
  "`@/tests` to `tests`",
  "`#/tests/…`",
  "every supported compiler, test runner, bundler, and editor/language server resolve it identically",
...
for (const [name, workflow] of [
  ...
  if (
    ...
    !workflow.includes(
      "report its assertion conventions and flag violations before reporting `PASS`"
    )
  ) {
    fail(`${name} must discover and report local test assertion conventions before PASS`)
  }
  if (
    !workflow.includes("Treat test setup as self-contained") ||
    !workflow.includes("stable environment setup")
  ) {
  ...
  if (
    !workflow.includes(
      "For generic parsing, traversal, tokenization, serialization, or equivalent plumbing"
    ) ||
  ...
  if (
    ...
    !workflow.includes(
      "import grouping, blank-line separation, test hierarchy, naming, and focused assertions"
    ) ||
  ...
  if (
    ...
    !workflow.includes(
      "before reporting `PASS`, name each independent responsibility and its side effects"
    )
  ) {
  ...
  if (
    ...
    fail(
      `${name} must review oversized mixed-responsibility files without requiring mechanical splits`
    )
  }
  if (
  ...
  if (
    ...
    fail(
      `${name} must flag hidden instance dependencies in private helpers without forcing artificial parameters`
    )
  }
  for (const requiredText of [
    "Start from Gold",
...
for (const [name, guide] of [
  ...
  if (
    ...
    fail(
      `${name} must reserve :art: for completed quality improvements without a narrower category`
    )
  }
  if (
    !guide.includes(
  ...
  if (
    guide.includes(":construction: Nest document routes") ||
    !guide.includes(":recycle: Nest document routes")
  ) {
...
for (const [name, workflow] of [
  ...
  if (
    !workflow.includes(
      "A standard source-plus-focused-test commit does not use `.commit-scope.json`"
    ) ||
    !workflow.includes(
...
const fixtureChecks = []
const packagedFixture = resolveFirstWorkflow(
  path.join(root, ".missing-local-workflows-fixture"),
  "learn"
)
if (packagedFixture.kind !== "fallback") {
  fail(
    `Packaged fallback fixture: expected fallback, got ${packagedFixture.kind} (${packagedFixture.path})`
  )
}
const commitPackagedFixture = resolveFirstWorkflow(
  path.join(root, ".missing-local-workflows-fixture"),
  "commit"
)
if (
  commitPackagedFixture.kind !== "fallback" ||
  commitPackagedFixture.path !== "skill://commit/workflow.md"
) {
  fail(
...
for (const fixture of fixtureChecks) {
  ...
  if (resolved.kind !== fixture.expectedKind) {
    fail(
      `${fixture.name}: expected ${fixture.expectedKind}, got ${resolved.kind} (${resolved.path})`
    )
  }
}
