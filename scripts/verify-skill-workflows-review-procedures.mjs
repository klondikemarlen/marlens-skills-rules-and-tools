...
export function verifyReviewProcedures({ read, fail }) {
  ...
  requireEvery(
    commentResolutionWorkflow,
    ['inspect the complete PR diff for the same underlying issue', 'dedicated `:ok_hand:` commit', 'PR scope checked'],
    (requiredText) => `comment resolution workflow must require ${requiredText}`,
    fail,
  );
  ...
  for (const [name, workflow] of pullRequestWorkflowVariants) {
    ...
    if (
      workflow.includes('Resolve every actionable review finding or comment before marking the PR ready or merging.')
    ) {
  ...
  if (
    normalizePullRequestWorkflow(authoritativePullRequestWorkflow) !== normalizePullRequestWorkflow(pullRequestWorkflow)
  ) {
    fail(
