...
for (const workflow of [authoritativeReview, packagedReview]) {
  ...
  assert.match(
    workflow,
    /record each candidate's evidence, responsibilities, boundary, and smallest safe next action/
  )
}
for (const workflow of [authoritativeSelfImprovement, packagedSelfImprovement]) {
  ...
  assert.match(
    workflow,
    /Revisit domain hypotheses before proposing refactoring or organization changes/
  )
}
const scenario = {
  folders: [
