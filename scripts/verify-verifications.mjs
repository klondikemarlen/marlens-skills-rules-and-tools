...
const automaticVerifications = verifications.filter(
  (verification) => verification.pathTriggers !== undefined
)
...
for (const id of ["marlens-rules:no-envrc-example", "marlens-rules:no-oversized-source-files"]) {
  assert.equal(
    verifications.find((verification) => verification.id === id)?.pathTriggers,
    undefined
  )
}
...
assert.ok(
  Array.isArray(testAlignmentVerification?.pathTriggers) &&
    testAlignmentVerification.pathTriggers.length > 0
)
...
assert.deepEqual(defaultFunctionExportsVerification?.pathTriggers, [
  "**/*.ts",
  "**/*.tsx",
  "**/*.cts",
  "**/*.mts",
