...
function ensureSafeToRewrite(target) {
  ...
  if (status) {
    throw new Error(
      "Worktree is not clean. Commit, stash, or discard unrelated changes before rewriting history."
    )
  }
  ...
  if (existsSync(rebaseDir) || existsSync(applyDir)) {
    throw new Error(
      "A rebase is already in progress. Finish it with git rebase --continue or abort it first."
    )
  }
}
...
try {
  const {
    ...
    target: targetArg,
    message,
  } = parseArgs(process.argv.slice(2), basename(process.argv[1]))
