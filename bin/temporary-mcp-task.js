...
export function nativeMcpConfigPaths({
  ...
  return [path.join(cwd, ".omp", "mcp.json"), userConfig].map((candidate) =>
...
export function temporaryMcpConfig(config, serverName) {
  ...
  const disabledServers = Array.isArray(temporaryConfig.disabledServers)
...
export function recoverStaleLease(configPath, lockPath) {
  ...
  writeAtomically(
    ...
    owner?.mode ?? statSync(backupPath).mode
  )
...
export function runTemporaryMcpTask({
  configPath,
  serverName,
  omp = "omp",
  prompt,
  ...
  if (!existsSync(resolvedConfigPath))
    throw new Error(`MCP config does not exist: ${resolvedConfigPath}`)
  ...
  try {
    ...
    const result = spawnSync(omp, ["--no-session", "--cwd", cwd, "-p", childPrompt], {
      stdio: "inherit",
    })
