  ...
  lines.forEach((lineText, index) => {
    ...
    for (const match of lineText.matchAll(/<!--\s*agent-guidance-audit:\s*([^>]+?)\s*-->/g)) {
      ...
      if (
        ...
        !parts[1] ||
        parts.length > 3 ||
        !SUPPRESSIBLE_CHECKS.has(parts[1])
      ) {
        results.push(
          finding(root, file, line, "audit-suppression", `invalid suppression ${directive}`)
        )
        continue
      }
...
function applySuppressions(root, file, findings, suppressions) {
  ...
  for (const item of findings) {
    const suppression = suppressions.find(
      ({ line, check, target }) =>
        line === item.line && check === item.check && (!target || target === item.target)
    )
    if (suppression) {
...
function scanFile(root, file) {
  ...
  for (const stale of STALE_LITERALS) {
    ...
    while (offset !== -1) {
      if (!isStaleLiteralExempt(text, offset, stale)) {
        results.push(
          finding(root, file, lineNumber(text, offset), stale.check, stale.detail, stale.needle)
        )
      }
  ...
  for (const match of text.matchAll(/`([^`\n]+)`/g)) {
    ...
    if (!isPathLike(token) || token.includes("*") || token.includes(" ") || token.includes("{"))
      continue
...
function markdownInventoryPaths(text) {
  const paths = new Set()
  for (const match of text.matchAll(
    /(?:`|\()([^`()\s]*(?:agents|docs)\/workflows\/[^`()\s]+\.md)(?:`|\))/g
  )) {
    paths.add(match[1])
  }
...
function scanWorkflowInventory(root) {
  ...
  for (const relativeDir of ["agents/workflows", "docs/workflows"]) {
    ...
    for (const item of listed) {
      const normalized = item.startsWith(relativeDir)
        ? item
        : `${relativeDir}/${path.basename(item)}`
      if (!actual.has(normalized)) {
        results.push(
          finding(
            ...
            1,
            "workflow-inventory",
            `inventory lists missing workflow ${item}`,
            item
          )
        )
      }
...
function scanMirror(pair) {
  ...
  if (!existsSync(left))
    return [finding(root, left, 1, "mirror-drift", `missing mirror path ${leftRaw}`, leftRaw)]
  if (!existsSync(right))
    return [finding(root, right, 1, "mirror-drift", `missing mirror path ${rightRaw}`, rightRaw)]
...
export function auditRoots(options) {
  ...
  for (const rootArg of options.roots) {
    ...
    if (!existsSync(root) || !lstatSync(root).isDirectory())
      throw new Error(`root is not a directory: ${rootArg}`)
...
function runSelfTest() {
  ...
  try {
    ...
    if (details.includes("docs/references/ok.md"))
      throw new Error("valid relative link was reported")

    writeFileSync(
      path.join(repo, "README.md"),
      "[ok](docs/references/ok.md) `docs/workflows/ok.md`\n"
    )
